...
const CreatePositions = function () {
    const form = $("#form_create_position");
            ...
            $.ajax({
                ...
                success: function (data) {
                    if(data.results.errors) {
                        show_error_message($('#id_name'), String(data.results.errors['name']).replace('Position', 'Cargo'));
                    } else {
                        Swal.fire({
                            title: '¡El Cargo se ha registrado satisfactoriamente!',
                            type: 'success',
                            timer: 2000,
