...
const CreateProcedures = function () {
    const form = $("#form_create_procedure");
    ...
    const initEvent = function () {
        ...
        $('#id_abbreviation').on('change', function () {
            if ($('#id_abbreviation').val() != '') {
                if(!$('#id_abbreviation').val().match(/^[0-9]+$/)) {
                    show_error_message($('#id_abbreviation'), 'Este campo debe complir con el formato ##');
                } else {
        ...
        $('#form_submit').click(function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    Swal.fire({
                        title: '¡El Procedimiento se ha registrado satisfactoriamente!',
                        type: 'success',
                        timer: 2000,
