...
const CreateAfectation = function () {
    const form = $("#form_create_afectation");
    ...
    const initEvent = function () {
        ...
        $('#id_afectacion').on('change', function () {
            if ($('#id_afectacion').val() != '') {
                hiden_error_message($('#id_afectacion'));
                
            } else {
                show_error_message($('#id_afectacion'), 'Este campo es requerido');
            }
        });
        ...
        $('#id_propuesto').on('change', function () {
            if ($('#id_propuesto').val() != '') {
                ...
                if($('#id_propuesto').val() == $('#id_responsable').val()) { 
                    show_error_message($('#id_propuesto'), 'El trabajador que reporta la incidencia no puede ser el encargado de la misma');
                    return;
                        }else{  
                    hiden_error_message($('#id_propuesto'));
                        }
        ...
        $('#id_responsable').on('change', function () {
            if ($('#id_responsable').val() != '') {
                ...
                hiden_error_message($('#id_propuesto'));
                if($('#id_propuesto').val() == $('#id_responsable').val()) { 
                    show_error_message($('#id_responsable'), 'El trabajador que reporta la incidencia no puede ser el encargado de la misma');
                    return;
                        }else{  
                    hiden_error_message($('#id_responsable'));
                        }
    ...
    const submitForm = function () {
        ...
        $('#form_submit').click(function () {
            ...
            if($('#id_trabajadorselect2-id_propuesto-container').val() == $('#select2-id_responsable-container').val()) { 
            ...
            $.ajax({
                ...
                success: function (data) {
                    Swal.fire({
                        title: '¡La afectación se ha creado satisfactoriamente!',
                        type: 'success',
                        timer: 2000,
