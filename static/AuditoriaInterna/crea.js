...
const CrearAudInt = function () {
    ...
    const submitForm = function () {
        ...
        $('#form_submit').click(function () {
            ...
            if($('#id_alcance').val() == ''){
                show_error_message($('#id_alcance'), 'Este campo es requerido');
                return;
            }
            ...
            if($('#id_fechainicio').val() == ''){
                ...
                return;
                   ...
                   if(Date.now() > Date.parse($('#id_fechafin').val())) { 
                       show_error_message($('#id_fechafin'), 'La fecha de fin de auditoría no puede ser menor que la fecha actual');
                       return;
                          }else{  
                        hiden_error_message($('#id_fechafin'));
                          }
                          ...
                          if(Date.now() < Date.parse($('#id_fechainicio').val())) { 
                            show_error_message($('#id_fechainicio'), 'La fecha de inicio de auditoría no puede ser mayor que la fecha actual');
                            return;
                               }else{  
                             hiden_error_message($('#id_fechainicio'));
                               }
